...
const TripDetails = ({ params }) => {
	...
	const getDetails = async () => {
		...
		if (res.ok) {
			...
			const existingTrips =
				JSON.parse(localStorage.getItem("tripIDs")) || [];
			if (!existingTrips.includes(json.trip.tripID)) {
				// append to the beginning
				existingTrips.unshift(json.trip.tripID);
				localStorage.setItem("tripIDs", JSON.stringify(existingTrips));
			}
			setData(json);
		} else {
			const json = await res.json();
			alert(json.message);
			// remove tripID from local storage
			const existingTrips =
				JSON.parse(localStorage.getItem("tripIDs")) || [];
			const updatedTrips = existingTrips.filter((trip) => trip != tripID);
		...
		<div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
			{data ? (
				<div className="bg-white p-6 rounded shadow-md w-full max-w-lg">
					...
					<p>
						<strong>Year:</strong> {mapping.years[data.trip.year]}
					</p>
					<p>
